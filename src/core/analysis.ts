...
const describeL1Coeff = (c: number): string => {
  const ord = gfOrder(c)
  const primitiveOrds: Record<number, string> = {
...
const NAMED_POLYS: Record<string, string> = {
...
const describeHigherOrder = (coeffs: number[], noisePercent: number): string => {
  if (coeffs.length === 0) return "L=0 trivial (constant zero sequence)"
  ...
  const isPrimitiveLead = gfOrder(constantTerm) === 255
...
export interface SegmentInfo {
  readonly offset:       number     // byte offset in original buffer
  readonly length:       number     // bytes in this segment
  readonly kind:         "lfsr" | "cyclic" | "raw"
  readonly L:            number | null  // LFSR order (null for raw/cyclic)
  readonly period:       number | null  // period for cyclic or computed LFSR period
  readonly coeffs:       number[]   // LFSR coefficients (empty for raw/cyclic)
  readonly noisePercent: number     // fraction of residual bytes (0 = perfect)
  readonly recognition:  string     // e.g. "PRBS-8 m-sequence", "repeating table", "no structure"
  readonly compressedSize: number   // wire bytes used for this chunk
}
...
export interface AnalysisResult {
  ...
  readonly structuredFraction: number  // fraction of bytes in LFSR/cyclic chunks
  readonly linearComplexity: number   // weighted average LFSR order (by segment length)
  readonly verdict:          string   // one-line human summary
}
...
const classifySegment = (chunk: LFSRChunk | CyclicChunk, offset: number): SegmentInfo => {
  ...
  const period = L === 1 ? gfOrder(lfsr.coeffs[0]!) : null

  // Recognition string
  let recognition = ""
  if (L === 1) {
    recognition = describeL1Coeff(lfsr.coeffs[0]!)
    if (gfOrder(lfsr.coeffs[0]!) === 255) {
...
const describeTransformChunk = (chunk: TransformChunk): { recognition: string; noisePercent: number } => {
  ...
  if (chunk.kind === "delta") {
    const labels: Record<number, string> = { 3: "XOR-diff", 4: "ADD-diff", 5: "XOR-2nd-diff" }
...
export const analyzeBuffer = (buf: Uint8Array, filename?: string): AnalysisResult => {
  ...
  for (const chunk of file.chunks) {
    ...
    if (seg.L !== null) lcSum += seg.L * seg.length
    offset += seg.length
  }
...
export const shouldCompress = (buf: Uint8Array): boolean => {
  if (buf.length < 4) return false
  const probe = buf.subarray(0, Math.min(buf.length, 256))
  const { length: L } = addon.bmSolve(Buffer.from(probe))
  if (L / probe.length <= 0.25) return true
  return shannonEntropy(buf) < 6.0
}

// Machine-readable JSON report — schema matches the C analyzer's --json output.
export const toJSON = (r: AnalysisResult): string =>
  JSON.stringify({
...
export const formatAnalysis = (r: AnalysisResult): string => {
  const lines: string[] = []
  const sep = "─".repeat(60)

  if (r.filename) lines.push(`File:        ${r.filename}`)
