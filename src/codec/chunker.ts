...
const linearComplexity = (buf: Uint8Array): number =>
  addon.bmSolve(Buffer.from(buf.subarray(0, Math.min(buf.length, LC_WINDOW)))).length
...
const modelStableSplit = (buf: Uint8Array): Uint8Array[] => {
  if (buf.length < MIN_CHUNK * 2) return [buf]
  if (linearComplexity(buf) <= MS_L_CAP) return [buf]  // already fits one model
...
export const adaptiveChunks = (buf: Uint8Array): Uint8Array[] => {
  if (buf.length <= MIN_CHUNK * 2) return [buf]
