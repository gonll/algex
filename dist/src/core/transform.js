...
exports.DELTA_TRANSFORMS = exports.deltaXor2Invert = exports.deltaXor2Apply = exports.deltaAdd1Invert = exports.deltaAdd1Apply = exports.deltaXor1Invert = exports.deltaXor1Apply = exports.DELTA_XOR2_ID = exports.DELTA_ADD1_ID = exports.DELTA_XOR1_ID = exports.shouldTryTransforms = exports.algebraicityScore = exports.baselineCost = exports.rawCost = exports.huffmanEstimate = void 0;
...
exports.huffmanEstimate = huffmanEstimate;
const rawCost = (x) => x.length * 8;
exports.rawCost = rawCost;
// Minimum achievable cost: a statistical codec (gzip) competes, not raw storage.
const baselineCost = (x) => Math.min((0, exports.rawCost)(x), (0, exports.huffmanEstimate)(x));
exports.baselineCost = baselineCost;
// ── Algebraicity score ────────────────────────────────────────────────────────
//
...
const algebraicityScore = (x, probe = ALG_PROBE) => {
    const len = Math.min(probe, x.length);
    if (len < ALG_WINDOW)
        return 0; // too short — assume structured
    let maxSlope = 0;
    const half = ALG_WINDOW / 2;
    for (let i = 0; i + ALG_WINDOW <= len; i += ALG_STEP) {
        const L0 = addon_1.addon.bmSolve(Buffer.from(x.subarray(i, i + half))).length;
        const L1 = addon_1.addon.bmSolve(Buffer.from(x.subarray(i + half, i + ALG_WINDOW))).length;
...
exports.algebraicityScore = algebraicityScore;
// ── Dual pre-gate ─────────────────────────────────────────────────────────────
...
const shouldTryTransforms = (x) => {
    if (x.length < 16)
        return false;
    if ((0, exports.huffmanEstimate)(x) <= ENTROPY_GATE * (0, exports.rawCost)(x))
        return false; // gate 1
    return (0, exports.algebraicityScore)(x) <= ALGEBRAICITY_GATE; // gate 2
};
exports.shouldTryTransforms = shouldTryTransforms;
...
exports.DELTA_XOR1_ID = 3; // XOR first difference:  z[i] = x[i] ^ x[i-1]
exports.DELTA_ADD1_ID = 4; // ADD first difference:  z[i] = (x[i] - x[i-1]) mod 256
exports.DELTA_XOR2_ID = 5; // XOR second difference: apply XOR-1 twice
// ── Delta apply / invert ──────────────────────────────────────────────────────
const deltaXor1Apply = (x) => {
...
exports.deltaXor1Apply = deltaXor1Apply;
const deltaXor1Invert = (x) => {
    const out = new Uint8Array(x.length);
    if (x.length === 0)
        return out;
    out[0] = x[0];
...
exports.deltaXor1Invert = deltaXor1Invert;
// ADD difference catches counter sequences linear over integers, not GF(2⁸).
const deltaAdd1Apply = (x) => {
    const out = new Uint8Array(x.length);
    if (x.length === 0)
        return out;
    out[0] = x[0];
    for (let i = 1; i < x.length; i++)
        out[i] = (x[i] - x[i - 1] + 256) & 0xff;
    return out;
};
exports.deltaAdd1Apply = deltaAdd1Apply;
const deltaAdd1Invert = (x) => {
    const out = new Uint8Array(x.length);
    if (x.length === 0)
        return out;
    out[0] = x[0];
    for (let i = 1; i < x.length; i++)
        out[i] = (x[i] + out[i - 1]) & 0xff;
    return out;
};
exports.deltaAdd1Invert = deltaAdd1Invert;
const deltaXor2Apply = (x) => (0, exports.deltaXor1Apply)((0, exports.deltaXor1Apply)(x));
exports.deltaXor2Apply = deltaXor2Apply;
const deltaXor2Invert = (x) => (0, exports.deltaXor1Invert)((0, exports.deltaXor1Invert)(x));
exports.deltaXor2Invert = deltaXor2Invert;
// ── Transform table ───────────────────────────────────────────────────────────
exports.DELTA_TRANSFORMS = [
    { id: exports.DELTA_XOR1_ID, apply: exports.deltaXor1Apply, invert: exports.deltaXor1Invert },
    { id: exports.DELTA_ADD1_ID, apply: exports.deltaAdd1Apply, invert: exports.deltaAdd1Invert },
    { id: exports.DELTA_XOR2_ID, apply: exports.deltaXor2Apply, invert: exports.deltaXor2Invert },
];
