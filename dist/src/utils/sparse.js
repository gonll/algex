...
exports.unpackResidual = exports.packedResidualSize = exports.packResidual = void 0;
...
const varintByteLen = (gap) => gap < 128 ? 1 : 2;
// Write a VarInt gap into buf at offset; returns bytes written.
const writeVarint = (buf, off, gap) => {
    if (gap < 128) {
        buf[off] = gap;
        return 1;
    }
...
const packResidual = (residual) => {
    if ((0, buffer_1.isAllZero)(residual))
        return new Uint8Array([0]);
    const pairs = [];
    residual.forEach((v, i) => { if (v !== 0)
        pairs.push([i, v]); });
...
exports.packedResidualSize = packedResidualSize;
// Read from buf at offset; returns [decoded residual, bytes consumed]
const unpackResidual = (buf, off, lfsrRegionLen) => {
    const kind = buf[off];
    if (kind === 0)
...
exports.unpackResidual = unpackResidual;
