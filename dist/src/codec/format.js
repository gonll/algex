...
exports.deserialize = exports.readChunkAt = exports.deserializeChunk = exports.serializeChunk = exports.serialize = void 0;
exports.streamDeserialize = streamDeserialize;
...
const wireResidual = (residual) => {
    const packed = (0, sparse_1.packResidual)(residual);
    const deflated = (0, zlib_1.deflateRawSync)(packed, { level: 9 });
    const brotlied = (0, zlib_1.brotliCompressSync)(packed, { params: { [zlib_1.constants.BROTLI_PARAM_QUALITY]: 6 } });
...
const preparedSimpleSize = (p) => {
    if (p.kind === "raw")
        return 1 + 4 + p.chunk.data.length;
    if (p.kind === "cyclic")
        return 1 + 4 + 2 + p.chunk.cycle.length;
    const { prefix, lfsr, init } = p.chunk;
    return 1 + 4 + 1 + prefix.length + 2 + lfsr.coeffs.length + init.length + p.rWire.length;
};
const writeSimpleChunk = (buf, view, p, off) => {
    const start = off;
    if (p.kind === "raw") {
        buf[off++] = KIND_RAW;
        view.setUint32(off, p.chunk.data.length);
        off += 4;
        buf.set(p.chunk.data, off);
        off += p.chunk.data.length;
        return off - start;
    }
    if (p.kind === "cyclic") {
        buf[off++] = KIND_CYCLIC;
        view.setUint32(off, p.chunk.originalLength);
        off += 4;
        view.setUint16(off, p.chunk.cycle.length);
        off += 2;
        buf.set(p.chunk.cycle, off);
        off += p.chunk.cycle.length;
        return off - start;
    }
    const { prefix, lfsr, init, originalLength } = p.chunk;
    buf[off++] = KIND_LFSR;
    view.setUint32(off, originalLength);
    off += 4;
    buf[off++] = prefix.length;
    buf.set(prefix, off);
    off += prefix.length;
    view.setUint16(off, lfsr.coeffs.length);
    off += 2;
    for (const c of lfsr.coeffs)
        buf[off++] = c;
    for (const v of init)
        buf[off++] = v;
    buf.set(p.rWire, off);
...
const serializeSimpleChunk = (chunk) => {
    let p;
    if (chunk.kind === "lfsr")
        p = { kind: "lfsr", chunk, rWire: wireResidual(chunk.residual) };
    else if (chunk.kind === "cyclic")
        p = { kind: "cyclic", chunk };
    else
        p = { kind: "raw", chunk };
...
const preparedSize = (p) => {
    if (p.kind === "raw")
        return 1 + 4 + p.chunk.data.length;
    if (p.kind === "cyclic")
        return 1 + 4 + 2 + p.chunk.cycle.length;
    if (p.kind === "delta")
        return 1 + 4 + 1 + 4 + p.innerBuf.length;
    if (p.kind === "affine")
        return 1 + 4 + 1 + 4 + p.innerBuf.length;
    if (p.kind === "interleave")
        return 1 + 4 + 1 + p.laneBufs.reduce((s, b) => s + 4 + b.length, 0);
    if (p.kind === "bitplane")
        return 1 + 4 + 1 + p.planeBufs.reduce((s, b) => s + 4 + b.length, 0);
    if (p.kind === "lfsr16") {
...
const writeChunk = (buf, view, p, off) => {
    const start = off;
    if (p.kind === "raw") {
        buf[off++] = KIND_RAW;
        view.setUint32(off, p.chunk.data.length);
    ...
    if (p.kind === "delta") {
        ...
        view.setUint32(off, p.innerBuf.length);
        off += 4;
        buf.set(p.innerBuf, off);
        off += p.innerBuf.length;
        return off - start;
    }
    if (p.kind === "affine") {
        buf[off++] = KIND_AFFINE;
    ...
    if (p.kind === "interleave") {
        ...
        for (const laneBuf of p.laneBufs) {
            view.setUint32(off, laneBuf.length);
            off += 4;
            buf.set(laneBuf, off);
            off += laneBuf.length;
        }
    ...
    if (p.kind === "bitplane") {
        ...
        for (const planeBuf of p.planeBufs) {
            view.setUint32(off, planeBuf.length);
            off += 4;
            buf.set(planeBuf, off);
            off += planeBuf.length;
        }
        return off - start;
    }
    if (p.kind === "lfsr16") {
        ...
        for (const c of coeffs) {
            view.setUint16(off, c);
            off += 2;
        }
        buf.set(seed, off);
        off += seed.length; // seed is already L16*2 raw bytes
        buf.set(p.rWire, off);
        off += p.rWire.length;
        return off - start;
    }
...
const serializeInnerChunk = (chunk) => {
...
const prepare = (chunk) => {
    if (chunk.kind === "lfsr")
        return { kind: "lfsr", chunk, rWire: wireResidual(chunk.residual) };
    if (chunk.kind === "cyclic")
        return { kind: "cyclic", chunk };
    if (chunk.kind === "lfsr16")
        return { kind: "lfsr16", chunk, rWire: wireResidual(chunk.residual) };
    if (chunk.kind === "affine")
        return { kind: "affine", chunk, innerBuf: serializeSimpleChunk(chunk.inner) };
    if (chunk.kind === "interleave")
        return { kind: "interleave", chunk, laneBufs: chunk.lanes.map(serializeSimpleChunk) };
    if (chunk.kind === "bitplane")
        return { kind: "bitplane", chunk, planeBufs: chunk.planes.map(serializeSimpleChunk) };
    // DeltaChunk.inner is NonDeltaChunk — may be interleave/bitplane (depth-2), use serializeInnerChunk
    if (chunk.kind === "delta")
        return { kind: "delta", chunk, innerBuf: serializeInnerChunk(chunk.inner) };
...
const serialize = (file) => {
    ...
    let off = 0;
    view.setUint32(off, MAGIC_V4);
    off += 4;
    view.setUint32(off, file.originalSize);
    off += 4;
    view.setUint32(off, n);
    off += 4;
    const chunkOffsets = [];
    for (let i = 0; i < n; i++) {
        ...
        off += payloadLen;
        view.setUint32(off, chunkCRC);
        off += 4;
    }
    buf[off++] = KIND_EOF;
    const indexOffset = off;
    view.setUint32(off, XDNI_MAGIC);
    off += 4;
    view.setUint32(off, n);
    off += 4;
    for (let i = 0; i < n; i++) {
        view.setUint32(off, chunkOffsets[i]);
        off += 4;
        const p = prepared[i];
        const origLen = p.kind === "raw"
            ? p.chunk.data.length
            : p.chunk.originalLength;
        view.setUint32(off, origLen);
        off += 4;
    }
    view.setUint32(off, indexOffset);
    off += 4;
    return buf;
};
...
exports.serializeChunk = serializeChunk;
const readChunkInner = (buf, off, isV4) => {
    const view = new DataView(buf.buffer, buf.byteOffset);
    const start = off;
    const kind = buf[off];
    if (kind === KIND_EOF)
        return [{ kind: "__eof__" }, 1];
    ...
    if (kind === KIND_RAW) {
        ...
        if (isV4) {
            const s = view.getUint32(off);
            off += 4;
            const a = crc32(buf.subarray(start, payloadEnd));
            if (s !== a)
                throw new Error(`CRC mismatch @${start}`);
        }
        return [{ kind: "raw", data }, off - start];
    }
    if (kind === KIND_CYCLIC) {
        const originalLength = view.getUint32(off);
        off += 4;
        const period = view.getUint16(off);
        off += 2;
        const cycle = buf.slice(off, off + period);
        off += period;
        const payloadEnd = off;
        if (isV4) {
            const s = view.getUint32(off);
    ...
    if (kind === KIND_DELTA) {
        ...
        const innerLen = view.getUint32(off);
        off += 4;
        const innerBuf = buf.slice(off, off + innerLen);
        off += innerLen;
        const payloadEnd = off;
        if (isV4) {
            const s = view.getUint32(off);
        ...
        const inner = readChunkInner(innerBuf, 0, false)[0];
    ...
    if (kind === KIND_INTERLEAVE) {
        ...
        const lanes = [];
        for (let j = 0; j < m; j++) {
            const laneLen = view.getUint32(off);
            off += 4;
            const laneBuf = buf.slice(off, off + laneLen);
            off += laneLen;
            lanes.push(readChunkInner(laneBuf, 0, false)[0]);
        }
        const payloadEnd = off;
        if (isV4) {
            const s = view.getUint32(off);
    ...
    if (kind === KIND_BITPLANE) {
        ...
        for (let j = 0; j < planeCount; j++) {
            const planeLen = view.getUint32(off);
            off += 4;
            const planeBuf = buf.slice(off, off + planeLen);
            off += planeLen;
            planes.push(readChunkInner(planeBuf, 0, false)[0]);
        }
        const payloadEnd = off;
        if (isV4) {
            const s = view.getUint32(off);
    ...
    if (kind === KIND_LFSR16) {
        ...
        const coeffs = [];
        for (let j = 0; j < L16; j++) {
            coeffs.push(view.getUint16(off));
            off += 2;
        }
        const seed = buf.slice(off, off + L16 * 2);
        off += L16 * 2;
        const lfsrRegionLen = originalLength; // LFSR16 covers the full chunk (no prefix)
        const flag = buf[off++];
        let residual;
        if (flag === RES_BROTLI) {
            const compLen = view.getUint32(off);
            off += 4;
            const plain = (0, zlib_1.brotliDecompressSync)(buf.slice(off, off + compLen));
            off += compLen;
            [residual] = (0, sparse_1.unpackResidual)(plain, 0, lfsrRegionLen);
        }
        else if (flag === RES_DEFLATED) {
            const deflLen = view.getUint32(off);
            off += 4;
            const plain = (0, zlib_1.inflateRawSync)(buf.slice(off, off + deflLen));
            off += deflLen;
            [residual] = (0, sparse_1.unpackResidual)(plain, 0, lfsrRegionLen);
        }
        else {
            const [res, consumed] = (0, sparse_1.unpackResidual)(buf, off, lfsrRegionLen);
            residual = res;
            off += consumed;
        }
        const payloadEnd = off;
    ...
    const prefix = buf.slice(off, off + prefixLen);
    off += prefixLen;
    const lfsrLen = view.getUint16(off);
    off += 2;
    const coeffs = Array.from(buf.subarray(off, off + lfsrLen));
    off += lfsrLen;
    const init = Array.from(buf.subarray(off, off + lfsrLen));
    off += lfsrLen;
    ...
    const payloadEnd = off;
    if (isV4) {
        const stored = view.getUint32(off);
        off += 4;
        const actual = crc32(buf.subarray(start, payloadEnd));
        if (stored !== actual)
            throw new Error(`CRC mismatch at offset ${start}: expected 0x${actual.toString(16)}, got 0x${stored.toString(16)}`);
    }
    return [
...
// ── deserializeChunk ──────────────────────────────────────────────────────────
const deserializeChunk = (buf) => readChunkInner(buf, 0, false)[0];
exports.deserializeChunk = deserializeChunk;
// ── readChunkAt (O(1) seek via XDNI index) ───────────────────────────────────
const readChunkAt = (buf, chunkIndex) => {
    if (buf.length < 8)
        return null;
    const view = new DataView(buf.buffer, buf.byteOffset);
    const indexOffset = view.getUint32(buf.length - 4);
    if (indexOffset + 4 > buf.length)
        return null;
    if (view.getUint32(indexOffset) !== XDNI_MAGIC)
        return null;
    ...
    return readChunkInner(buf, chunkStart, true)[0];
};
exports.readChunkAt = readChunkAt;
// ── deserialize ───────────────────────────────────────────────────────────────
const deserialize = (buf) => {
    const view = new DataView(buf.buffer, buf.byteOffset);
    let off = 0;
    const magic = view.getUint32(off);
    off += 4;
    ...
    const originalSize = view.getUint32(off);
    off += 4;
    const chunkCount = view.getUint32(off);
    off += 4;
    const chunks = [];
    if (isV4) {
...
// ── streamDeserialize ─────────────────────────────────────────────────────────
function* streamDeserialize(buf) {
    const view = new DataView(buf.buffer, buf.byteOffset);
    let off = 0;
    const magic = view.getUint32(off);
    off += 4;
    ...
    const _originalSize = view.getUint32(off);
    off += 4;
    const chunkCount = view.getUint32(off);
    off += 4;
    if (isV4) {
